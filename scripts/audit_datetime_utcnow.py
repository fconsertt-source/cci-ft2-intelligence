...
def detect_context(path: Path, line: str, next_line: Optional[str] = None) -> Tuple[str, str]:
    """
...
def format_report(results: List[Dict], stats: Dict) -> str:
    ...
    lines = []
    lines.append("=" * 80)
    lines.append(f"📋 تقرير تحليل استخدامات utc_now_iso() - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("=" * 80)
    lines.append("")
    ...
    lines.append("       return datetime.now(timezone.utc).isoformat(timespec=timespec).replace('+00:00', 'Z')")
    lines.append("   ```")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description='تحليل استخدامات utc_now_iso() - المرحلة الأولى')
    parser.add_argument('--output', '-o', type=str, help='حفظ التقرير في ملف')
    parser.add_argument('--verbose', '-v', action='store_true', help='عرض تفاصيل إضافية')
            ...
            for file in files:
                ...
                for i, line in enumerate(lines):
                    if UTC_PATTERN.search(line):
                        ...
                        context, compare_type = detect_context(file_path, line, next_line)
    ...
    if stats['total'] > 0:
        ...
        print("   للمرحلة الثانية (التطبيق)، استخدم: python scripts/fix_datetime.py --apply")

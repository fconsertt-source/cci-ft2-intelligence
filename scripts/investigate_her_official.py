...
def load_real_spec(vaccine_type):
    ...
    # استخدم آلية تحميل المواصفات الحقيقية من المشروع
    from src.infrastructure.adapters.json_vaccine_spec_repository import \
        JsonVaccineSpecRepository

    repo = JsonVaccineSpecRepository()
    return repo.get_spec(vaccine_type)
...
def investigate_her_calculation():
    ...
    for name, readings in [("high", readings_high), ("medium", readings_medium)]:
        ...
        for r in readings:
            ...
            print(
                f"  q10={q10:.4f}, hours={r.duration_hours}, contribution={contribution:.2f}"
            )
...
def document_findings(results):
    ...
    print(
        "3. الحد الأقصى للـ HER في النظام:",

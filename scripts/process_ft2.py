...
def process_single_file(
    ...
    try:
        ...
        if archived_hash != file_hash:
            raise RuntimeError(f"Hash mismatch after archival: {file_hash} != {archived_hash}")
...
def main():
    ...
    logger.info(f"Ledger State: {state.entry_count} entries, last_hash={state.last_entry_hash[:16] if state.last_entry_hash else 'None'}...")

...
class EvaluateColdChainSafetyResponse:
    ...
    has_who_heat_exposure: bool = False
    # ✅ الحقل المفقود — كان يُمرَّر من from_context لكنه غير معرَّف
    has_heat_duration_breach: bool = False
    her_ratio: float = 0.0
    ccm_index: str = "0"
    ...
    reference_audit: dict = field(default_factory=dict)

    @classmethod
    def from_context(cls, ctx, stats: dict):
        return cls(
            center_id=ctx.id or "UNKNOWN",
            ...
            has_heat_duration_breach=stats.get("has_heat_duration_breach", False),
            ...
            reference_audit=stats.get("reference_audit", {}),
        )
...
class LedgerEntry:
    ...
    def _to_serializable_dict(self, include_current_hash: bool = False) -> dict:
        ...
        data = {
            ...
            'event_type': self.event_type.name,  # ✅ تحويل Enum إلى string
        ...
        return {
            ...
            if v is not None or k == 'previous_hash'
        }
    ...
    def from_dict(cls, data: dict) -> LedgerEntry:
        ...
        if isinstance(filtered_data.get('event_type'), str):
            try:
                filtered_data['event_type'] = LedgerEvent[filtered_data['event_type']]
            except KeyError:
                raise ValueError(f"Invalid event_type: {filtered_data.get('event_type')}")
        
        # تصفية الحقول لتناسب constructor
        constructor_fields = {f.name for f in fields(cls) if f.init}
        return cls(**{k: v for k, v in filtered_data.items() if k in constructor_fields})

    ...
    def _parse_berlinger(self, path: Path) -> List[FT2EntryDTO]:
        ...
        for line in lines:
            ...
            if 'date:' in stripped.lower():
                date_match = re.search(r'date:\s*(\d{4}-\d{2}-\d{2})', stripped, re.IGNORECASE)
                if date_match:
                    current_date = date_match.group(1)
                continue
            
            if 'avrg t:' in stripped.lower():
                temp_match = re.search(r'avrg\s*t:\s*([+-]?\d+\.?\d*)', stripped, re.IGNORECASE)
                if temp_match:

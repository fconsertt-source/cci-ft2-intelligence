...
from src.domain.evidence.evidence_integrity_report import EvidenceIntegrityReport, EvidenceGrade
...
class HybridEvidenceValidator:
    ...
    def validate(
        ...
        if txt_verification.status in [VerificationStatus.CRYPTO_FAILURE, 
                                        VerificationStatus.INVALID_SIGNATURE]:
            logger.warning("توقيع غير صالح - رفض المعالجة: %s, الحالة: %s", 
                          txt_path.name, txt_verification.status)
            return self._build_rejection_report(
                txt_path,
    ...
    def _ensure_normalized_data_bytes(self, verification: VerificationResult) -> VerificationResult:
        ...
        if isinstance(raw, str):
            ...
            object.__setattr__(verification, 'normalized_data', raw.encode("utf-8", errors="replace"))
            return verification
        
        logger.warning("normalized_data is unexpected type %s, using repr", type(raw))
        object.__setattr__(verification, 'normalized_data', repr(raw).encode("utf-8", errors="replace"))
        return verification
    ...
    def _build_rejection_report(
        ...
        """بناء تقرير رفض موحد"""
        serial = getattr(txt_path, 'stem', 'UNKNOWN').split('_')[0] if txt_path else 'UNKNOWN'
    ...
    def _log_to_ledger(self, txt_path: Path, report: EvidenceIntegrityReport) -> None:
        ...
        try:
            event_type = LedgerEvent.AUTHENTICITY_VERIFIED if not report.is_rejected \

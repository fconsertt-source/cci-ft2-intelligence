...
class LicenseGuard:
    ...
    def _handle_trial_status(self, status: TrialStatus) -> None:
        """Centralized status handling for license events.

        This was refactored to reduce repeated condition logic across
        multiple guard checks (similar-by-purpose code paths: ACTIVE/EXPIRED/TAMPERED).
        Origin: business rule from license policy + audit trail requirements.
        """
        status_actions = {
            TrialStatus.ACTIVE: None,
            TrialStatus.EXPIRED: lambda: (_ for _ in ()).throw(
                LicenseExpiredError("Trial period expired")
            ),
            TrialStatus.TAMPERED: lambda: (_ for _ in ()).throw(
                IntegrityError("Time rollback detected")
            ),
        }

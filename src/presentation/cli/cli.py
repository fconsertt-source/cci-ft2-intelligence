...
def evaluate(
    ...
    if not data_path.exists():
        typer.echo(MessageMap.get("DEVICE_DATA_SOURCE_MISSING", path=str(data_path)), err=True)
        raise typer.Exit(code=1)
...
def report(
    ...
    if not input_path.exists():
        typer.echo(MessageMap.get("DEVICE_DATA_SOURCE_MISSING", path=str(input_path)), err=True)
        raise typer.Exit(code=1)
...
def generate_device_report(
    ...
    output_path: Path = typer.Option(
        ...
        "--output", "-o", help="مسار حفظ تقرير الجهاز"
    ),
    data_path: Path = typer.Option(
        "ft2_data.json", "--data", "-d", help="مسار البيانات الوسيطة"
    ),
    ...
    if not data_path.exists():
        typer.echo(
            MessageMap.get("DEVICE_DATA_SOURCE_MISSING", path=str(data_path)),  # ← رسالة مخصصة
            err=True
        )
        raise typer.Exit(code=1)
    ...
    try:
        ...
        report = uc.execute(device_id=device_id)
        ...
        typer.echo(MessageMap.get(
            "DEVICE_REPORT_GENERATED",
            device_id=device_id,
...
def generate_all_device_reports(
    ...
    try:
        ...
        typer.echo(MessageMap.get("DEVICE_REPORTS_GENERATION_FAILED", error=str(e)), err=True)
        raise typer.Exit(code=1)
...
def verify_official(file_path: Path):
    ...
    try:
        ...
        logger.info("Official verification completed for %s - Status: %s", 
                   file_path, result.status.value)

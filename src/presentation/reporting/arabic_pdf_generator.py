...
def create_arabic_pdf():
    ...
    try:
        ...
        # عناوين الجدول
        headers = ["رقم المركز", "اسم المركز", "القرار", "متوسط الحرارة", "الحالة"]
        ...
        # إنشاء الجدول
        table = Table(
            table_data, colWidths=[3 * cm, 5 * cm, 3.5 * cm, 3 * cm, 2.5 * cm]
        )
        ...
        summary_data = [
            ...
            [
                "المراكز تحت المراقبة",
                str(warning),
                f"{warning/total*100:.1f}%" if total > 0 else "0%",
            ],
            [
                "المراكز المرفوضة",
                str(rejected),
                f"{rejected/total*100:.1f}%" if total > 0 else "0%",
            ],
            [
                "لا بيانات",
                str(no_data),
                f"{no_data/total*100:.1f}%" if total > 0 else "0%",
            ],
        ]

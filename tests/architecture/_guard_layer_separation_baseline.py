...
def test_trial_policy_has_no_thermal_dependencies():
    ...
    forbidden_modules = {
        "her", "ccm", "q10", "thermal", "regulatory", "calculator", "domain.services"
    }

    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            if any(mod in node.module.lower() for mod in forbidden_modules):
                raise AssertionError(f"TrialPolicy imports forbidden module: {node.module}")
        ...
        if isinstance(node, ast.Attribute):
            if hasattr(node.value, 'id') and node.value.id == "src":
                if "thermal" in node.attr.lower():
                    raise AssertionError(f"Found thermal reference in TrialPolicy: {node.attr}")

...
def test_execute_calls_dependencies_correctly(guard): # 2. نمرر guard هنا كمعامل
    ...
    sample_records = [
        MagicMock(timestamp=datetime(2021, 12, 2), temperature=2.5, duration_minutes=1440.0, vaccine_type="Hepatitis_B")
    ]
    mock_repo.get_device_history.return_value = sample_records

    sample_spec = MagicMock(vaccine_type="Hepatitis_B", rationale="Normal Hepatitis B storage")
...
def test_weakest_link_logic(guard):
    ...
    records = [
        MagicMock(temperature=2.5, duration_minutes=1440.0, vaccine_type="HepB"), # SAFE
        MagicMock(temperature=9.5, duration_minutes=1440.0, vaccine_type="HepB"), # DISCARD (Worst)
        MagicMock(temperature=3.0, duration_minutes=1440.0, vaccine_type="HepB")  # SAFE
    ]
...
@patch(
    "src.application.use_cases.generate_device_report_uc.LicenseGuard.ensure_active"
)    

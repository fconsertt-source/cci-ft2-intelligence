...
class TestEvaluateColdChainSafetyUseCase:
    ...
    def test_execute_freeze_violation(self):
        ...
        # قراءات عادية
        for i in range(4):
            readings_list.append(TemperatureReading(value=4.0, timestamp=base_time + timedelta(minutes=15*i), device_id="DEV_FREEZE"))
        
        # قراءات تجميد (أقل من -0.5 لفترة)
        for i in range(4):
            readings_list.append(TemperatureReading(value=-2.0, timestamp=base_time + timedelta(minutes=15*(4+i)), device_id="DEV_FREEZE"))
    ...
    def test_execute_empty_readings(self):
        ...
        use_case = EvaluateColdChainSafetyUseCase()
        request = EvaluateColdChainSafetyRequest(center_id="C_EMPTY", center_name="Empty", readings=(), temperature_ranges={'min': 2, 'max': 8})

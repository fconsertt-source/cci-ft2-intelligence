...
def assess(vaccine_assessment_service, fixed_today):
    ...
    def _assess(vaccine, readings):
        return vaccine_assessment_service.assess(vaccine, readings, reference_date=fixed_today)
    return _assess
...
class TestVaccinePipelineIntegration:
    ...
    def test_expired_vaccine_discarded_first(
        ...
        assert "انتهت الصلاحية" in result.decision_detail or "منتهية الصلاحية" in result.decision_detail
    ...
    def test_her_between_1_and_1_5_partial(
        ...
        partial = next(v for v in equipment_vaccines if v.batch_number == "B2024-PARTIAL")
    ...
    def test_full_pipeline_advanced_report(
        self, equipment_vaccines, temperature_readings, freezer_temperature_readings,
        partial_temperature_readings, high_her_temperature_readings,  # ✅ إضافة
        assess, report_generator, tmp_path
    ):

...
def test_import_uc_delegates_to_ports():
    ...
    mock_writer.write.assert_called_once_with(mock_reader.read.return_value, output_path)  # ← data first, destination second
    mock_logger.info.assert_any_call(f"Importing FT2 data from '{input_path}' to '{output_path}'")
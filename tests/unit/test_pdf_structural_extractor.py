...
from src.infrastructure.security.pdf_structural_extractor import PDFStructuralExtractor
...
t Acc: 0
        ...
        assert serial == "130600112748", f"الـ Serial المتوقع: 130600112748، المستخرج: {serial}"
        
        # اختبار استخراج درجات الحرارة
        min_temp = extractor._extract_min_temperature(mock_pdf_text)
        assert min_temp == Decimal("1.8"), f"أقل حرارة متوقعة: 1.8، المستخرجة: {min_temp}"
        
        max_temp = extractor._extract_max_temperature(mock_pdf_text)
        assert max_temp == Decimal("6.8"), f"أعلى حرارة متوقعة: 6.8، المستخرجة: {max_temp}"
    ...
    @pytest.mark.skipif(not (PDF_DIR / "130600112748_report.pdf").exists(),

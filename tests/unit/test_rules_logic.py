...
class TestRulesLogic(unittest.TestCase):
    ...
    def test_expiry_rule_rejection(self):
        ...
        self.center.expiry_date = (datetime.now() - timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
        ...
        self.assertTrue(
            any("لقاح منتهي الصلاحية" in r for r in self.center.decision_reasons)
        )

    def test_expiry_rule_valid(self):
        """اختبار قبول اللقاح ساري الصلاحية"""
        self.center.expiry_date = (datetime.now() + timedelta(days=30)).strftime(
            "%Y-%m-%d"
        )
        self.center.ft2_entries = [MockEntry(temperature=5.0, duration_minutes=100)]
    ...
    def test_expiry_rule_invalid_format(self):
        ...
        self.assertTrue(
            any(
                "تنسيق تاريخ صلاحية غير صالح" in r for r in self.center.decision_reasons
            )
        )
    ...
    def test_heat_critical_threshold_violation(self):
        ...
        apply_rules(
            self.center, extra_stats={"critical_temp_limit": 12.0, "max_temp": 13.0}
        )
